...
export function validarObservador(value: unknown): { data: Observador } | { error: string; status: 400 } {
  ...
  for (const field of ['horaFinal', 'sede', 'jornada', 'grupo', 'acudiente', 'cedulaAcudiente'] as const) {
    data[field] = typeof raw[field] === 'string' ? raw[field].trim() : '';
  }
  if (typeof raw.fechaRegistro === 'string') {
    if (Number.isNaN(new Date(raw.fechaRegistro).getTime())) return fail('Fecha del equipo inválida.');
...
export function datosReporteObservador(data: Observador) {
  return {
    ...
    fechaHecho: data.fechaRegistro || new Date(`${data.fecha}T${data.horaInicio}:00-05:00`).toISOString(),
    lugar: data.sede || 'Sin registrar',
